<!-- ─────────────────────────────────────────────────────────────────────
     PHOEBE AGENT — project scaffold state + TODO seams (first pass)
     ─────────────────────────────────────────────────────────────────────
This env trains the Phoebe agent: input = a reconciled trip scenario
(state + blocker flags); output = {blocker, action, message} JSON.

WIRED (first pass, connects + dry-run validates):
  - environment.py      PhoebeEnv (single-turn), first-pass reward = blocker.kind
                        match + action.kind match. OUTPUT_SCHEMA defined.
  - gen_dataset.py      distills 3 starter rows from phoebe._SAMPLES using the
                        REAL app.agents.phoebe.diagnose / decide_action (labels
                        match production). Run: python -m training.gen_dataset
  - dataset/train.jsonl 3 rows (issue / timing / person blockers).
  - configs/{sft,rl,opd}.toml  base=Qwen/Qwen3.5-4B, [environment].id, SFT first.

TODO SEAMS for the next subagent (each marked TODO(subagent) in code):
  1. Full scenario generator with diversity (all blocker kinds × group sizes ×
     budget spreads) + a decontaminated eval split.  → training/scenario_gen.py
     + training/gen_dataset.py; ablation eval in training/run_ablation.py.
  2. Ground-truth reward in environment.score_response: 0.45·blocker + 0.25·
     action + 0.30·message-rubric − length-penalty, with OUTPUT_SCHEMA bound as
...
Everything here is isolated in training/ — the bot/runtime is untouched.
Flash CLI runs from the isolated ../.venv-flash (Python 3.12); the bot venv is
3.13. Note: on this machine the python.org 3.12 needs certifi's CA bundle —
export SSL_CERT_FILE=$(../.venv-flash/bin/python -c 'import certifi;print(certifi.where())')
before flash commands, or login/push hit SSL errors.
────────────────────────────────────────────────────────────────────── -->
