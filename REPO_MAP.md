...
| `db.py`       | MongoDB Atlas layer: `card_sessions` mirror + append-only `analytics` (swipe dwell/drag/velocity, card views, link-outs). Never raises; retry cooldown + event buffering when Atlas flakes. | `cards.py`, `api.py` |
