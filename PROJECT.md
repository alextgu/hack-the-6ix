Plan That Trip to Japan — Project Reference for a hackathon.
...
LayerWhatOwnershipLive inventoryStay22 API — prices/availability across Expedia, Booking, Hotels.com, VrboDependency (data source)Chat → constraintsParse group chat into {city, dates, budget, size, vibe}Ours — the BrainPreference aggregationReconcile conflicting per-person inputs into one planOurs — hardest partHealth engineTwo-axis health (physical + mental), delta-based decayOursPet renderTelegram bot + pet visual posted on state changeOursEventsPhoebe agent books/holds activities on commitPhoebe SDK


4. Build areas


Read — parse group chat → {city, dates, budget, size}, reconcile conflicting answers into one plan. (Hardest; the Freesolo model lives here.)
Price — pull live Stay22 price/availability → drive the physical bar → fire the real booking on commit.
Nudge — track chat engagement → drive the mental bar. Phoebe's home: the diagnose-target-convince agent (§6) that finds the blocker and resolves it.
Render — run the Telegram bot, draw the pet's moods, post the updated image on every state change.
Explore — the onboarding split (experienced vs. new) and the Tinder-style map exploration input.
Speak — the pet's voice; the deathbed call to the chat when a bar bottoms out. Only after 1–4 work end to end.
...
A person. A holdout who hasn't confirmed and everyone's waiting on, or the person whose budget/dates are the binding constraint (e.g. their $1,200 cap sits below the group median, so no hotel fits everyone).
Timing. No overlapping date window, or the group keeps deferring while a price/availability deadline approaches.
The exact issue. Budget conflict, date conflict, destination split (Tokyo vs. Osaka), or pure decision paralysis — everyone agrees, nobody commits.
...
ElevenLabs — powers: the pet's voice — it calls the group when a bar bottoms out, and you can talk back.
Wins by: being a two-way agent, not one-way TTS. It takes at least one real action mid-call (hold a room, kick off a user's onboarding, DM the holdout). Their rubric rewards agentic depth + real-time dialogue over playback.
